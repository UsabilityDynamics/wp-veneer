        ...
        foreach( (array) $_settings as $key => $value ) {
          if( !defined( strtoupper( $key ) ) ) {
            ...
            if( in_array( strtoupper( $key ), $this->protected_constants ) ){
              throw new \Exception( 'The constant "' . $protected_constant . '" is defined in composer.json - it\'s autogenerated. Please remove that line.' );
            }
          }
        }
